# (PART) DATA PROCESSING IN R {-}

# Tidying Bioinformatics Output {#data-tidying}

## Output from `mothur` pipeline
...
### Creating `mothur` otu and taxonomy tables {#mothur-otu-taxonomy}
```{r}
library(tidyverse)
...
## Output from `qiime2` pipeline

### Creating `qiime2` taxa abundance table {#qiime2-otu-taxonomy}
...
## Output from `metaphlan` pipeline
...
resave(m3_merged_abund_table, file = "RDataRDS/saved_objects.RData")
```
...
```
## Output from `humann` pipeline
```{block, type="tmbinfo", echo=T}
> Processing `humann` output will be added in the near future. 
```
