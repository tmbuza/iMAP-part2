...
## Explore QIIME2 OTU Clustering Methods
### De novo clustering
...
## Create tidy qiime2 taxa abundance table
...
q2_genus_lineage <- read_tsv("data/q2-transformed-tables/q2_feature_tax_abund.tsv", skip = 0, show_col_types = FALSE)[-1,] %>% 
  rename(feature = id,
         taxonomy = Taxon) %>% 
...
colnames(q2_genus_lineage)

saveRDS(q2_genus_lineage, "RDataRDS/q2_genus_lineage.rds")
resave(q2_genus_lineage, file = "~/Dropbox/CDILLC/GIT_REPOS/smda-end2end/RDataRDS/saved_objects.RData")
```
...
q2_otutable <- readRDS("RDataRDS/q2_genus_lineage.rds") %>%
  select(-all_of(taxlevels))

cat("Dimension of the otutable\n")
dim(q2_otutable)
cat("\nStructure of the otutable\n")
head(q2_otutable)[, 1:5] %>% as.data.frame()  
...
q2_taxonomy <- readRDS("RDataRDS/q2_genus_lineage.rds") %>%
  rename_all(tolower) %>%
...
head(q2_taxonomy) %>% as.data.frame() 
...
```{bash}
cp ~/Dropbox/CDILLC/GIT_REPOS/iMAP_part2/RDataRDS/q2_taxonomy.rds ~/Dropbox/CDILLC/GIT_REPOS/iMAP_part3/RDataRDS/
cp ~/Dropbox/CDILLC/GIT_REPOS/iMAP_part2/RDataRDS/q2_otutable.rds ~/Dropbox/CDILLC/GIT_REPOS/iMAP_part3/RDataRDS/
```
