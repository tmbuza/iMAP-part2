...
## Expected tabulated results
1. Operational Taxonomic Units (OTUs).
2. Amplicon Sequence Variants (ASVs).
3. Phylotypes.
4. Conserved taxonomy.

## Download trained classifer
...
## Implementing `mothur` pipeline interactively
- We recommend to familiarize with the tutorials detailed in [MiSeq SOP](https://mothur.org/wiki/miseq_sop/).
...
Let's get started! Start by running one line at a time to see how it goes. See another option at the end of the script.
...
```bash
mothur code/mothur_code.batch # if mothur is in the path
./mothur code/mothur_code.batch # on mac/linux if the executable `mothur` is in the working directory.
./mothur.exe code/mothur_code.batch # on Windows machins if the executable `mothur.exe` is in the working directory.
```
...
## Getting tidy OTUs and taxonomy
```{r message=FALSE, warning=FALSE}
...
# Taxonomic Profiling of Metagenomics Sequencing Data {#metaphlan-taxonomic-profiling}
## Expected tabulated results
1. Relative taxonomic profiles with taxonomy or
2. Relative taxonomic profiles with read count and taxonomy if setting *-t rel_ab_w_read_stats*.
